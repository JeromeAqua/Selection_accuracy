...
nc <- nc_open("LSQ_daily_ftle.nc") #"http://basin.ceoe.udel.edu/thredds/dodsC/AQUAGLOBAL8DAY9KM.nc") #"MODIS_8day_sst.nc")
...
for (runnumber in 1:100){
  ...
  for (concentration in c(20,10,5,2,1,0.75,0.5,0.25,0)){
    ...
    if (concentration==20){Simulation <- Simulationstart
    ...
    for(i in 2:durtrack){ 
      ...
      SST <- t(ncvar_get(nc, varid = "ftle", start = c(1,1,satellitetime), count = c(-1,-1,1))) #get the chl ocx of the correct day -still called sst because easier not to change the name
      ...
      if (sum(SST, na.rm = T)>0){   #to avoid bugs when no data
        ...
        D <- D + STEP(concentration)#Change strength of the selection, in m -- % 2000 for high selection, 10000 for low selection
      } else {
  ...
  save.image(file = "FAkeanimals_ftle.RData")
  print(runnumber)
}
nc_close(nc)
...
P <-  ggplot(subset(Simulation, Simulation$kappa==2), aes(x = longitude %%360, y = latitude)) + geom_tile(data = Pacific, aes(x = x %%360, y = y, fill = z)) +
...
tiff('map_k2_sst.tiff', units="cm", width=10, height=16, res=800, compression = 'lzw')
P
dev.off()
