...
nc <- nc_open("MODIS_8day_sst.nc") #"http://basin.ceoe.udel.edu/thredds/dodsC/AQUAGLOBAL8DAY9KM.nc") #"MODIS_8day_sst.nc")
...
for (runnumber in 1:100){
  ...
  for (concentration in 0){
    ...
    for(i in 2:durtrack){ 
      ...
      # SST <- t(ncvar_get(nc, varid = "sst", start = c(1,1,satellitetime), count = c(-1,-1,1))) #get the sst of the correct day
      ...
      # if (sum(SST, na.rm = T)>0){   #to avoid bugs when no data
        ...
        B <- ANGLE(concentration) #+B# Change strength of the selection, in degrees -- % 5 for high selection, 1.5 for low selection
        D <- STEP(concentration) + 20000# D + STEP(concentration)#Change strength of the selection, in m -- % 2000 for high selection, 10000 for low selection
      # } else {
  ...
  save.image(file = "Fakeanimals_SST_0C.RData")
  print(runnumber)
}
nc_close(nc)
...
xlims <- c(213,238) #range(c(min(Simulation$longitude,na.rm = T), max(Simulation$longitude, na.rm = T)))#c(-65 %%360,130))
ylims <- c(5, 44) #range(c(min(Simulation$latitude, na.rm = T), max(Simulation$latitude, na.rm = T)))#c(-50, 65))
...
tiff('map_k0_C.tiff', units="px", width=3779, height=5984, res=800, compression = 'lzw')
P
dev.off()
...
# save.image(file = "Synthetic_sst.RData")
#sendmail('jpinti@udel.edu', 'jpinti@udel.edu', 'Biased walks for sst', '100 tracks wouhou!',control=list(smtpServer="mail.udel.edu"))
