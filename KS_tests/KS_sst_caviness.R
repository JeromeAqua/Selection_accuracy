...
load("/lustre/scratch/jpinti/Accuracy/Synthetic_SST_onlysst_v2.RData")
# load("/lustre/scratch/jpinti/Accuracy/Synthetic_all_sst_0.RData")
CUE <- "sst"
  ...
  for (S in sigmavector){
    ...
    for (ntracks in NTRACKS){
      ...
      repeat { #Repeat loop so that we can do it 50 times easily
        ...
        #Select the corresponding WM_sst values
        ...
        k <- ks.test(TRACKS$WM_sst,Brownian$WM_sst, alternative = ALT)
        D1 <- k$statistic
        p1 <- k$p.value
        
        # k <- ks.test(TRACKS$WM_sst,Levy$WM_sst, alternative = ALT)
        D2 <- k$statistic
        p2 <- k$p.value
        
        k <- ks.test(TRACKS$WM_sst,CRW$WM_sst, alternative = ALT)
        D3 <- k$statistic
        p3 <- k$p.value
        
        k <- ks.test(TRACKS$WM_sst,JD$WM_sst, alternative = ALT)
        D4 <- k$statistic
        p4 <- k$p.value
        
        k <- ks.test(TRACKS$WM_sst,c(JD$WM_sst,CRW$WM_sst,Brownian$WM_sst), alternative = ALT)
        D5 <- k$statistic
        p5 <- k$p.value
