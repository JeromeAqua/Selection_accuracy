...
library("boot")
 ...
 load("/lustre/scratch/jpinti/Accuracy/Synthetic_LCSgaps_onlylcs.RData")
 # load("/lustre/scratch/jpinti/Accuracy/Synthetic_LCS_onlylcs_v2.RData")
  #load("/lustre/scratch/jpinti/Accuracy/Synthetic_all_LCS_0.RData")
#load("/lustre/scratch/jpinti/Accuracy/Synthetic_all_LCS_0C.RData")

CUE <- "ftle"

# #remove these lines below if not needed
# outT$cue <- "ftle"
# outB$cue <- "ftle"
# outL$cue <- "ftle"
# outC$cue <- "ftle"
...
kappavector <- c(0.25, 0, 2, 0.75, 20,10,5,1,0.5) # 20 10 5 2 1 0.75 0.5 0.25 0 ##HERE WE DO IT ONE BY ONE ON BIOHEN TO HAVE THE RESULTS FASTER
...
for (K in kappavector){
  ...
  for (S in sigmavector){
    ...
    for (ntracks in NTRACKS){
      ...
      Dthresh <- 0.05 #threshold for considering that there is selection
      compt <- 0
      repeat { #Repeat loop so that we can do it N times easily
        
        #First we select ntracks events 
        Eventswewant <- sample(1:100, ntracks)
        # Ids <- outT$event_id[outT$kappa == K & outT$sigma==sigma]
        
        #Select the corresponding WM_ftle values
        ...
        k <- ks.test(TRACKS$WM_ftle,Brownian$WM_ftle, alternative = ALT)
         D1 <- k$statistic
        p1 <- k$p.value
        
        # k <- ks.test(TRACKS$WM_ftle,Levy$WM_ftle, alternative = ALT)
         D2 <- k$statistic
        p2 <- k$p.value
        
        k <- ks.test(TRACKS$WM_ftle,CRW$WM_ftle, alternative = ALT)
         D3 <- k$statistic
        p3 <- k$p.value
        
        k <- ks.test(TRACKS$WM_ftle,JD$WM_ftle, alternative = ALT)
         D4 <- k$statistic
        p4 <- k$p.value
        
        k <- ks.test(TRACKS$WM_ftle,c(JD$WM_ftle,CRW$WM_ftle,Brownian$WM_ftle), alternative = ALT)
         D5 <- k$statistic
        p5 <- k$p.value
    ...
    # tiff(paste(glue("Boosted_pvals_K{K}_S{S}_less_{CUE}.tiff")), units="cm", width=15, height=10, res=800, compression = 'lzw')
    # print(Qabove)
    # dev.off()
    # tiff(paste(glue("Boosted_pvals_K{K}_S{S}_greater_{CUE}.tiff")), units="cm", width=15, height=10, res=800, compression = 'lzw')
    # print(Qbelow)
    # dev.off()
  ...
  save(Alltesttracks, Allbootstrapestimates,file = paste(glue("Boostrapestimates_{CUE}_GAPS_K{K}.RData")))
